...
app.post('/proyecto/eliminar/:id', (req, res) => {
	const proyectoId = req.params.id; // ID del proyecto a eliminar
	const userId = req.session.user_id; // ID del usuario autenticado

	if (req.session.loggedin) {
		// Verificar si el proyecto pertenece al usuario
		connection.query('SELECT * FROM proyecto WHERE id = ? AND user_id = ?', [proyectoId, userId], (error, results) => {
			if (error) {
				return res.status(500).send('Error en la consulta a la base de datos');
			}

			if (results.length > 0) {
				// El proyecto pertenece al usuario, proceder con la eliminación
				connection.query('DELETE FROM proyecto WHERE id = ?', [proyectoId], (error) => {
					if (error) {
						return res.status(500).send('Error al eliminar el proyecto');
					}

					// Redirigir a la página de proyectos después de eliminarlo
					res.redirect('/proyecto');
				});
			} else {
				// El proyecto no pertenece al usuario
				res.status(403).send('No tienes permiso para eliminar este proyecto');
			}
		});
	} else {
